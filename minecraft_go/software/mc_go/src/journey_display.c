...
void displayElapsedTime(Time *elapsedTime, int start_x_coord, int y_coord) {
	...
	while (buffer[i]) {
		printf("%c", buffer[i]);
		graphics_char_font_4(start_x_coord + i * 15, y_coord, RED,
...
void displayMenu(Time *time1, double distance, float speed, int kill_count) {
	...
	while (buffer[i]) {
		printf("%c", buffer[i]);
		graphics_char_font_5(start_x_coord + i * 32, y_coord, WHITE,
				GRAPHICS_BACKGROUND_COLOUR, buffer[i], TRUE);
		i++;
	}
...
void displayFight(int x, int y) {
	...
	while (buffer[i]) {
		printf("%c", buffer[i]);
		graphics_char_font_1((x+25) + i * 10, (y+25), GREEN,
...
void displayConnect(int x, int y) {
	...
	while (buffer[i]) {
		printf("%c", buffer[i]);
		graphics_char_font_1((x+15) + i * 10, (y+25), BLACK,
				GRAPHICS_BACKGROUND_COLOUR, buffer[i], FALSE);
		i++;
	}
}
...
void displaySmash() {
	...
	while (buffer[i]) {
		printf("%c", buffer[i]);
		graphics_char_font_5(250 + i * 32, 225, GREEN,
				GRAPHICS_BACKGROUND_COLOUR, buffer[i], TRUE);
		i++;
	}
}
...
void displayLose() {
	...
	while (buffer[i]) {
		printf("%c", buffer[i]);
		graphics_char_font_5(250 + i * 32, 225, RED, GRAPHICS_BACKGROUND_COLOUR,
				buffer[i], FALSE);
		i++;
	}
}
...
void displayTextBox(char* text, int text_color, int border_color, int box_color) {
	...
	while (buffer[i]) {
		printf("%c", buffer[i]);
		graphics_char_font_5(250 + i * 32, 225, text_color,
				GRAPHICS_BACKGROUND_COLOUR, buffer[i], FALSE);
		i++;
	}
}
...
void displayTouchPrompt() {
	...
	while (buffer[i]) {
		printf("%c", buffer[i]);
		graphics_char_font_3(200 + i * 24, 225, YELLOW,
				GRAPHICS_BACKGROUND_COLOUR, buffer[i], FALSE);
		i++;
	}
}
